...
async function main() {
    ...
    const Y_STAKING_POOL = new ethers.Contract(PREANNUAL_YFI_STAKING_POOL, CREAM_REWARD_POOL_ABI, App.provider);
    ...
    const totalSupplyY = await CREAM_TOKEN.balanceOf(PREANNUAL_YFI_STAKING_POOL) / 1e18;
    const totalStakedYAmount = await Y_TOKEN.balanceOf(PREANNUAL_YFI_STAKING_POOL) / 1e18;
    ...
    _print("========== STAKING =========")
    _print(`There are total   : ${totalSupplyY} YFARMER issued by PERENNIAL YFI YFARMER.`);
    _print(`There are total   : ${totalStakedYAmount} YFI staked in ygov's PERENNIAL YFI staking pool.`);
