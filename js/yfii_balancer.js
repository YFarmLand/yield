...
async function main() {
    ...
    _print("========== PRICES ==========")
    _print(`1 XFI  = $${YFIIPrice}`);
    _print(`1 USDC   = $${DAIPrice}\n`);
    _print(`1 BPT   = [${YFIPerBPT} XFI, ${DAIPerBPT} USDC]`);
    _print(`        = ${toDollar(YFIPerBPT * YFIIPrice + DAIPerBPT * DAIPrice)}\n`);

    _print("========== STAKING =========")
    _print(`There are total   : ${totalBPTAmount} BPT issued by XFI DAI Balancer Pool.`);
    _print(`There are total   : ${totalStakedBPTAmount} BPT staked in XFI's BPT staking pool.`);
    ...
    _print(`                  = [${YFIPerBPT * stakedBPTAmount} XFI, ${DAIPerBPT * stakedBPTAmount} USDC]`);
    ...
    _print(`Claimable Rewards : ${toFixed(earnedYFI, 4)} XFI = ${toDollar(earnedYFI * YFIIPrice)}`);
    _print(`Weekly estimate   : ${toFixed(rewardPerToken * stakedBPTAmount, 2)} XFI = ${toDollar(rewardPerToken * stakedBPTAmount * YFIIPrice)} (out of total ${weekly_reward} XFI)`)
