...
async function main() {
    ...
    const Y_STAKING_POOL = new ethers.Contract(BIANNUAL_WETH_STAKING_POOL, CREAM_REWARD_POOL_ABI, App.provider);
    ...
    const totalSupplyY = await CREAM_TOKEN.balanceOf(BIANNUAL_WETH_STAKING_POOL) / 1e18;
    const totalStakedYAmount = await Y_TOKEN.balanceOf(BIANNUAL_WETH_STAKING_POOL) / 1e18;
    ...
    _print("========== STAKING =========")
    _print(`There are total   : ${totalSupplyY} YFARMER issued by BIANNUAL WETH YFARMER.`);
    _print(`There are total   : ${totalStakedYAmount} WETH staked in ygov's BIANNUAL WETH staking pool.`);
